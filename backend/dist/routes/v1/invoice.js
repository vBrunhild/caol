...
router.get('/', (req, res) => {
    ...
    if (startIssueDate && !isoDateRegex.test(startIssueDate)) {
        return res.status(400).json({
            error: 'Invalid startRequestDate format. Use YYYY-MM-DD format (e.g., 2024-01-01)',
        });
    }
    if (endIssueDate && !isoDateRegex.test(endIssueDate)) {
        return res.status(400).json({
    ...
    if (Array.isArray(serviceOrderId)) {
        serviceOrderIdList.push(...serviceOrderId.filter(id => typeof id === "string"));
    }
