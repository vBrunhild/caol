...
import { PaginationResult, ConsultantMonthlyTotal, User } from '../../types.js';
...
router.get('/monthly-totals', (req, res) => {
  ...
  let stmt = db.prepare<(string | number)[], ConsultantMonthlyTotal>(finalQuery);
  let monthly = stmt.all(...queryParams);

  let result: PaginationResult<ConsultantMonthlyTotal> = {
      limit,
      offset,
      total,
      hasNext,
